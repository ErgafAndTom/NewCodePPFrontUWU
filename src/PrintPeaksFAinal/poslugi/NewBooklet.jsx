...
const NewBooklet = ({
    ...
    useEffect(() => {
        let bigValue = materialAndDrukFront.big !== "Не потрібно" ? {
            count: Number(materialAndDrukFront.big),
            pricePerUnit: 7,
            totalPrice: (7 * Number(materialAndDrukFront.big) * count).toFixed(2)
        } : "Не потрібно";
        ...
        console.log('Data sent to server:', JSON.stringify(dataToSend, null, 2));
        axios.post(`/calc/pricing`, dataToSend)
            .then(response => {
                console.log('Server response:', response.data);
                const prices = response.data.prices;
                
                // Якщо сервер не повернув об'єкт big, створюємо його самі
                if (bigValue !== "Не потрібно" && !prices.big) {
                    prices.big = {
                        count: count * Number(bigValue.count),
                        pricePerUnit: 7, // Базова ціна за згинання
                        totalPrice: (7 * count * Number(bigValue.count)).toFixed(2)
                    };
                }
                
                setPricesThis(prices);
                setError(null);
            })
            .catch(error => {
                setError(error)
    ...
    }, [size, material, color, lamination, cute, cuteLocal, holes, holesR, count, porizka, materialAndDrukFront, materialAndDrukBack, pereplet]);
    ...
    const addNewOrderUnit = e => {
        ...
        // Перевірка та обробка згинання
        let bigValue = "Не потрібно";
        if (materialAndDrukFront.big && materialAndDrukFront.big !== "Не потрібно" && materialAndDrukFront.big !== "") {
            const bigCount = Number(materialAndDrukFront.big);
            const totalBigCount = bigCount * Number(count);
            ...
            if (bigCount > 0) {
                bigValue = {
                    ...
                    count: totalBigCount,
                    pricePerUnit: 7,
                    totalPrice: (7 * totalBigCount).toFixed(2),
                    material: "Згинання",
                    materialId: 999,
                    typeUse: "Згинання буклету"
                };
            }
        }

        console.log('Big value before sending:', bigValue);
        
        const { materialAndDrukFront: _, ...cleanedMaterialAndDrukFront } = materialAndDrukFront;
        
        let dataToSend = {
            orderId: thisOrder.id,
            toCalc: {
                nameOrderUnit: "Буклет",
                type: "Note",
                size: size,
                material: {
                    type: "Не потрібно",
        ...
        console.log('Full data structure being sent:', JSON.stringify(dataToSend, null, 2));
                    ...
                    <div className="d-flex flex-column" style={{
                        ...
                        <div className="d-flex">
                            <div className="m-auto text-center fontProductName">
                                Буклет / Автореферат / Конспект / Звіт / Журнал / Зін
                            </div>
                            <div
                                    ...
                                    <div className="">
                                        ...
                                        <div className="d-flex " style={{}}>
                                            <div className="d-flex flex-column">
                                                ...
                                                <div className="fontInfoForPricing">
                                                    Матеріали: {pricesThis.priceMaterialFront} грн * {pricesThis.sheetCount} шт = {(pricesThis.priceMaterialFront * pricesThis.sheetCount).toFixed(2)} грн
                                                </div>
                                                <div className="fontInfoForPricing">
                                                ...
                                                {materialAndDrukFront.big !== "Не потрібно" && (
                                                    <div className="fontInfoForPricing">
                                                        Згинання: {pricesThis?.big?.pricePerUnit || 0} грн * {pricesThis?.big?.count || 0} шт = {pricesThis?.big?.totalPrice || 0} грн
                                                    </div>
                                                )}
                                                <div className="fontInfoForPricing">
